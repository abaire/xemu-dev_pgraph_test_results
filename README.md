...
3. Ensure you have Python 3.12 or newer installed. (The run scripts will automatically set up a virtual environment and install dependencies on first execution.)
...
2. Use `./user_scripts/run.sh` (macOS/Linux) or `user_scripts\run.bat` (Windows) to run the [nxdk_pgraph_tests](https://github.com/abaire/nxdk_pgraph_tests) against your development xemu build. If running on a reasonably powerful machine you may wish to use the `-S` parameter to parallelize execution. See `./user_scripts/run.sh --help` (or `user_scripts\run.bat --help`).
...
During active development, it may be useful to filter the set of tests being run to those expected to change. See `./user_scripts/run.sh --help` for details.
