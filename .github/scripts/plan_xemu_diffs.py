...
from xemu_pgraph_ci_tools.xemu_diffs import identify_missing_xemu_diffs
...
def main() -> int:
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Clear existing comparison outputs to force re-running all diffs",
    )
    ...
    if not args.baseline_dir or not os.path.isdir(args.baseline_dir) or not os.path.isdir(args.results_dir):
        ...
        matrix: dict[str, list[int]] = {"shard": []}
    ...
    if args.force and os.path.isdir(args.output_dir):
        logger.info("Clearing %s due to --force", args.output_dir)
        shutil.rmtree(args.output_dir, ignore_errors=True)

    registry, tasks = identify_missing_xemu_diffs(
        results_dir=args.results_dir,
        golden_dir=args.baseline_dir,
